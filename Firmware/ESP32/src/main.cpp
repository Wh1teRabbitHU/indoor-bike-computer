...
static inline uint32_t calculateCRC(uint8_t* data, uint32_t dataLength) {
...
static inline uint32_t read32BitData(char* buffer, uint8_t start) {
    uint32_t data = 0;

    data |= buffer[start] << 24;
    data |= buffer[start + 1] << 16;
    data |= buffer[start + 2] << 8;
    data |= buffer[start + 3];
