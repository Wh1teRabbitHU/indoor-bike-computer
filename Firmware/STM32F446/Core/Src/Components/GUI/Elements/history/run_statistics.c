...
RunStatistics RunStatistics_create(RunStatistics_Config * config) {
    RunStatistics runsStatistics = {0};

    lv_obj_t * box = lv_obj_create(config->screen);

    lv_obj_set_width(box, RUN_STATISTICS_WIDTH);
    lv_obj_set_height(box, RUN_STATISTICS_HEIGHT);
    lv_obj_set_pos(box, config->x, config->y);
    lv_obj_set_style_pad_all(box, RUN_STATISTICS_MARGIN, LV_PART_MAIN);
    ...
    lv_obj_set_width(runsLabel, RUN_STATISTICS_WIDTH - 8);
    lv_obj_set_height(runsLabel, RUN_STATISTICS_LABEL_HEIGHT);
    ...
    lv_obj_align(distanceSumLabel, LV_ALIGN_TOP_LEFT, 0, RUN_STATISTICS_LABEL_HEIGHT);
    lv_obj_set_width(distanceSumLabel, RUN_STATISTICS_WIDTH - 8);
    lv_obj_set_height(runsLabel, RUN_STATISTICS_LABEL_HEIGHT);
    ...
    lv_obj_align(sessionsLengthLabel, LV_ALIGN_TOP_LEFT, 0, RUN_STATISTICS_LABEL_HEIGHT * 2);
    lv_obj_set_width(sessionsLengthLabel, RUN_STATISTICS_WIDTH - 8);
    lv_obj_set_height(runsLabel, RUN_STATISTICS_LABEL_HEIGHT);
