...
void Bike_setInfo(char* info) { TabView_Main_getState()->infoMessage = info; }
void Bike_setError(char* error) { TabView_Main_getState()->errorMessage = error; }
void Bike_clearInfo(void) { TabView_Main_getState()->infoMessage = NULL; }
void Bike_clearError(void) { TabView_Main_getState()->errorMessage = NULL; }
void Bike_setTime(RTC_TimeTypeDef* rtcTime) {
    sprintf(timeBuffer, "%02d:%02d:%02d", rtcTime->Hours, rtcTime->Minutes, rtcTime->Seconds);

    TabView_Main_getState()->time = timeBuffer;
    TabView_Main_getState()->updateChart = 1;
}

void Bike_updateDifficulty(void) {
    difficulty = MCP3421_readMeasurement();
    TabView_Main_getState()->difficulty = difficulty;
}
